...
proc deserializeMessage*(serializedMessage: openArray[byte]): Message =
    assert len(serializedMessage) == payloadSize, "Serialized message must be exactly " & $payloadSize & " bytes"
...
proc serializeHop*(hop: Hop): seq[byte] =
    assert len(hop.MultiAddress) == addrSize, "MultiAddress must be exactly " & $addrSize & " bytes"
    result = hop.MultiAddress
...
proc initRoutingInfo*(address: Hop, delay: seq[byte], gamma: seq[byte], beta: seq[byte]): RoutingInfo =
  result.Addr = address
  result.Delay = delay
  result.Gamma = gamma
  result.Beta = beta
...
proc deserializeSphinxPacket*(data: openArray[byte]): SphinxPacket =
    assert len(data) == packetSize, "Sphinx packet size must be exactly " & $packetSize & " bytes"
