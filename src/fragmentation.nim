...
proc initMessageChunk*(paddingLength: uint16, data: seq[byte], seqNo: uint32 ): MessageChunk =
...
proc serializeMessageChunk*(msgChunk: MessageChunk): seq[byte] = 
  ...
  assert len(msgChunk.data) == dataSize, "Padded data must be exactly " & $dataSize & " bytes"
...
proc padAndChunkMessage*(messageBytes: seq[byte], peerId: PeerId): seq[MessageChunk] =
  ...
  # Split to chunks
  let totalChunks = max(1, ceilDiv(len(messageBytes), dataSize)) # Ensure at least one chunk is generated
