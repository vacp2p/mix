...
suite "Mix Node Tests":
  ...
  test "get_mix_node_by_index":
    ...
    for i in 0..<count:
      ...
      let (multiAddr, mixPubKey, mixPrivKey, libp2pPubKey,
        libp2pPrivKey) = getMixNodeInfo(node)
      ...
      assert libp2pPrivKey.getBytes().len == SkRawPrivateKeySize,
          "Libp2p private key of node " & $i & " is not " &
  ...
  test "find_mix_node_by_peer_id":
    for i in 0..<count:
      ...
      let (fMultiAddr, fMixPubKey, fMixPrivKey, fLibp2pPubKey,
        fLibp2pPrivKey) = getMixNodeInfo(foundNode)
  ...
  test "write_and_read_mix_node_info":
    for i in 0..<count:
      ...
      let (rMultiAddr, rMixPubKey, rMixPrivKey, rLibp2pPubKey,
        rLibp2pPrivKey) = getMixNodeInfo(readNode)
