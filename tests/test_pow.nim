...
suite "pow_tests":
  test "int_to_bytes_conversion":
    ...
    assert intToBytes(65535, 2) == @[255.byte, 255.byte],
      "Failed to convert int64 to 2 bytes"
    assert intToBytes(65536, 4) == @[0.byte, 0.byte, 1.byte, 0.byte],
      "Failed to convert int64 to 4 bytes"

    # Test edge case with maximum int64 value
    let maxInt64 = int64.high
    assert intToBytes(maxInt64, 8) ==
      @[
        0xFF.byte, 0xFF.byte, 0xFF.byte, 0xFF.byte, 0xFF.byte, 0xFF.byte, 0xFF.byte,
        0x7F.byte,
      ], "Failed to convert max int64 value to 8 bytes"
  ...
  test "pow_computation_verification":
    ...
    assert result.len == message.len + 8 + 4,
      "Result length should be message length + timestamp (8 bytes) + nonce (4 bytes)"
  ...
  test "timestamp_outside_window":
    ...
    assert not verifyPow(invalidData),
      "verifyPow should return false for inputData with timestamp outside the valid window"
  ...
  test "invalid_nonce_check":
    ...
    assert not verifyPow(invalidData),
      "verifyPow should return false for inputData with invalid hash"
