...
suite "pow_tests":
    ...
    test "valid_hash_check":
        ...
        assert isValidHash(hashWithDifficultyLevel), "Hash with " & $difficultyLevel & " leading zeros must be valid"
        
        let hashWithLessDifficultyLevel = createDummyHash(difficultyLevel - 1)
        assert not isValidHash(hashWithLessDifficultyLevel), "Hash with " & $(difficultyLevel - 1) & " leading zeros should not be valid"
