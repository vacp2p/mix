...
suite "Fragmentation":
  ...
  test "pad_and_chunk_small_message":
    ...
    let messageBytesLen = len(cast[seq[byte]](message))
  ...
  test "pad_and_chunk_large_message":
    let message = repeat('A', messageSize * 2 + 10)
    let messageBytesLen = len(cast[seq[byte]](message))
  ...
  test "chunk_sequence_numbers_are_consecutive":
    let message = repeat('A', messageSize * 3)
    let messageBytesLen = len(cast[seq[byte]](message))
